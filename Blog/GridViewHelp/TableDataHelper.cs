...
namespace GridViewHelper
{
    ...
    public static class TableDataHelper
    {
        ...
        /// <summary>
        /// HtmlHelp的拓展方法，根据提供的数据源实现数据的表格显示
        /// </summary>
        ...
        public static string TableDataGridView<T>(this HtmlHelper helper, object data, string[] columns,TableDataOption options)
        {
            ...
            //显示分页部分数据
            RenderPageRow<T>(helper, writer, (TableDataPagedList<T>)items, columns.Count());
        ...
        //输出控制分页部分
        private static void RenderPageRow<T>(HtmlHelper helper, HtmlTextWriter writer, TableDataPagedList<T> items, int columnCount)
        {
        ...
        public static void RenderHeader(HtmlHelper helper,HtmlTextWriter writer,string[] columns,TableDataOption options)
        {
            ...
            foreach(string columnName in columns)
            {
                ...
                //<a href="/currentAction?sort=columnsName">options.Columns[i]</a>
                writer.Write(link);
