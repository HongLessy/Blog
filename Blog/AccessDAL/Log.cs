    ...
    public class Log:ILog
    {
        ...
        public LogEntity SelectLogByID(int t_id)
        {
            ...
            using(sdr=OLEDBHelp.GetReader("select * from Logs where id="+t_id))
            {
                if(sdr.Read())
                {
                    ...
                    t_Log.Operatevent=(string)sdr.GetValue(3);
                }
            }
            sdr.Close(); 
            return t_Log;
        ...
        public int InsertLog(LogEntity t_Log)
        {
          //定义插入数据的参数数组
              ...
              new OleDbParameter("@Operatevent",t_Log.Operatevent)
           };
           int i=OLEDBHelp.GetExecute("insert into Logs values (@Id,@Author_id,@Date,@Operatevent)", p) ;
           return i;
        }
        ...
        public int UpdateLog(LogEntity t_Log)
        {
            ...
            int i=OLEDBHelp.GetExecute("update Logs set id=@Id,date=@Date,operatevent=@Operatevent where id=@Id", p) ;
            return i;
        }
